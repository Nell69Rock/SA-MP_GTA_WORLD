...
Drivers_Init()
{
	...
	for(new i = 0; i < sizeof(gDestinationList); i ++) 
		minsize += strlen(gDestinationList[i][destName]);
	
	if(sizeof(gDestinationDialogSTR) < minsize) 
		printf("[DRIVERS] Warning: Higher the size of gDestinationDialogSTR from %d to at least %d. Not all Destinations can be displayed.", sizeof(gDestinationDialogSTR), minsize);
